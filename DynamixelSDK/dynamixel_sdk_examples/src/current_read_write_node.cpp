...
: Node("current_read_write_node") // Initialize the Node with the name "current_read_write_node"
{
...
void ReadWriteNode::setCurrentControl() {
  ...
  if (dxl_comm_result != COMM_SUCCESS) {
    RCLCPP_ERROR(rclcpp::get_logger("current_read_write_node"), "Failed to set Current Control Mode.");
  } else {
    RCLCPP_INFO(rclcpp::get_logger("current_read_write_node"), "Succeeded to set Current Control Mode.");
  }
}

void ReadWriteNode::setPositionControl() {
...
/*
  ...
  if (dxl_comm_result != COMM_SUCCESS) {
    RCLCPP_ERROR(rclcpp::get_logger("current_read_write_node"), "Failed to set Position Control Mode.");
  } else {
    RCLCPP_INFO(rclcpp::get_logger("current_read_write_node"), "Succeeded to set Position Control Mode.");
  }
*/
  dxl_comm_result = packetHandler->write1ByteTxRx(
    portHandler,
  ...
  if (dxl_comm_result != COMM_SUCCESS) {
    RCLCPP_ERROR(rclcpp::get_logger("current_read_write_node"), "Failed to enable torque.");
  } else {
    RCLCPP_INFO(rclcpp::get_logger("current_read_write_node"), "Succeeded to enable torque.");
  }
}
...
int main(int argc, char * argv[])
{
  ...
  if (dxl_comm_result == false) {
    RCLCPP_ERROR(rclcpp::get_logger("current_read_write_node"), "Failed to open the port!");
    return -1;
  } else {
    RCLCPP_INFO(rclcpp::get_logger("current_read_write_node"), "Succeeded to open the port.");
  }

  // Set the baudrate of the serial port (use DYNAMIXEL Baudrate)
  dxl_comm_result = portHandler->setBaudRate(BAUDRATE);
  if (dxl_comm_result == false) {
    RCLCPP_ERROR(rclcpp::get_logger("current_read_write_node"), "Failed to set the baudrate!");
    return -1;
  } else {
    RCLCPP_INFO(rclcpp::get_logger("current_read_write_node"), "Succeeded to set the baudrate.");
  }
